  ...
  for (const position of positions) {
    ...
    if (maxHoldDays) {
      ...
      if (daysSinceBuy >= maxHoldDays) {
        await executeSell(supabase, user_id, symbol, quantity, currentPrice, 'max_hold_days_reached')
    ...
    for (const rule of rules) {
      ...
      if (triggered) {
        ...
        await executeSell(supabase, user_id, symbol, sellQty, currentPrice, `rule: ${condition} ${threshold}%`)
